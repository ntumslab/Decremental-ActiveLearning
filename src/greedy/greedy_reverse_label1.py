'''
    ...
    approach1 : the same data point may be selected repeatly
'''
...
write_svm_file (train_x, train_y, tmp_file_path + pure_data_name+'-temp-r.train')

# ------------------------------------
cmd = liblinear_incdec_train_path + ' -s 2 -q ' +  tmp_file_path + pure_data_name + '-temp-r.train'
subprocess.call (cmd.split())
m = load_model ( pure_data_name+'-temp-r.train.model')
...
cmd = 'rm ' + pure_data_name + '-temp-r.train.model'
subprocess.call (cmd.split())
# ------------------------------------
...
for it in range(data_num_size+1):
    acc_rank = []
    write_svm_file (train_x_copy, train_y_copy, tmp_file_path + pure_data_name+'-temp2-r.train')
    cmd = liblinear_incdec_train_path + ' -s 2  -q ' +  tmp_file_path + pure_data_name + '-temp2-r.train'
    subprocess.call (cmd.split())

    for i in range(len(train_y_copy)):
        cmd = 'cp ' \
              +  pure_data_name + '-temp2-r.train.model'+ ' ' \
              +  pure_data_name + '-temp3-r.train.model'
        subprocess.call (cmd.split())
        ...
        temp_train_y.pop(i)
        write_svm_file (temp_train_x, temp_train_y,  tmp_file_path + pure_data_name+'-'+'temp3-r.train')
        cmd = liblinear_incdec_train_path \
                + ' -s 2 -q -i ' \
                +  pure_data_name+'-temp3-r.train.model' + ' ' \
                + tmp_file_path+pure_data_name+'-temp3-r.train'
        subprocess.call (cmd.split())
        m2 = load_model (pure_data_name+'-'+'temp3-r.train.model')
        ...
        cmd = 'rm ' + pure_data_name + '-temp3-r.train.model'
        subprocess.call (cmd.split())

    cmd = 'rm ' + pure_data_name + '-temp2-r.train.model'
    subprocess.call (cmd.split())
...
plt.title(pure_data_name + '.r_out_' + ('%.3f' % base_acc_out))
...
plt.savefig(output_Folder + pure_data_name + '_Eout_r_out.png')

'''
    Run experiment: Greedy reverse data point in 'two class' training set.
    approach2 : the data point onlybe selected once.
'''
...
data_num_size = int(len(train_y)*0.5)
#base_size = int(len(train_y)*0.1/data_num_size) + 1
# base_size = 1 ;

write_svm_file (train_x, train_y, tmp_file_path + pure_data_name+'-temp-r2.train')

# ------------------------------------
cmd = liblinear_incdec_train_path + ' -s 2 -q ' +  tmp_file_path + pure_data_name + '-temp-r2.train'
subprocess.call (cmd.split())
m = load_model ( pure_data_name+'-temp-r2.train.model')
...
cmd = 'rm ' + pure_data_name + '-temp-r2.train.model'
subprocess.call (cmd.split())
# ------------------------------------
...
for it in range(data_num_size+1):
    acc_rank = []
    write_svm_file (train_x_copy, train_y_copy, tmp_file_path + pure_data_name+'-temp2-r2.train')
    cmd = liblinear_incdec_train_path + ' -s 2  -q ' +  tmp_file_path + pure_data_name + '-temp2-r2.train'
    subprocess.call (cmd.split())

    for i, index in enumerate(remain_dict):
        cmd = 'cp ' \
              +  pure_data_name + '-temp2-r2.train.model'+ ' ' \
              +  pure_data_name + '-temp3-r2.train.model'
        subprocess.call (cmd.split())

        temp_train_x = copy.deepcopy(train_x_copy)
        temp_train_x.pop(index)
        temp_train_y = copy.deepcopy(train_y_copy)
        temp_train_y.pop(index)
        write_svm_file (temp_train_x, temp_train_y,  tmp_file_path + pure_data_name+'-temp3-r2.train')
        cmd = liblinear_incdec_train_path \
                + ' -s 2 -q -i ' \
                +  pure_data_name+'-temp3-r2.train.model' + ' ' \
                + tmp_file_path+pure_data_name+'-temp3-r2.train'
        subprocess.call (cmd.split())
        m2 = load_model (pure_data_name+'-temp3-r2.train.model')
        ...
        acc_rank.append((p_acc_out[0], p_acc_in[0], index))

        cmd = 'rm ' + pure_data_name + '-temp3-r2.train.model'
        subprocess.call (cmd.split())

    cmd = 'rm ' + pure_data_name + '-temp2-r2.train.model'
    subprocess.call (cmd.split())
    (max_acc_out, max_acc_in, reverse_num) = sorted(acc_rank, key=lambda x:x[0])[-1] #sort by p_acc_out
    #remove_order.append(remain_dict.pop(remove_num)) #since the real remove index is in the remain_dict
    train_y_copy[reverse_num] *= (-1.0)
    remove_order.append(reverse_num)
    remain_dict.remove(reverse_num)
...
plt.title(pure_data_name + '.r2_out_' + ('%.3f' % base_acc_out))
...
plt.savefig(output_Folder + pure_data_name + '_Eout_r2_out.png')

plt.cla()

# ----------------------------------------------------------
step = round(0.5 / len(E_in_1), 5)
